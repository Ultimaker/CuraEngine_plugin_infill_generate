...
template<>
struct tag<infill::geometry::polyline<>>
{
    using type = linestring_tag;
};

template<>
struct point_order<infill::geometry::polygon_outer<>>
{
    static const order_selector value = clockwise;
};

template<>
struct closure<infill::geometry::polygon_outer<>>
{
    static const closure_selector value = open; // TODO: closed?, but when it is closed the order of the points changes
};

template<>
struct tag<infill::geometry::polygon_outer<>>
{
    using type = ring_tag;
};

template<>
struct point_order<infill::geometry::polygon_inner<>>
{
    static const order_selector value = counterclockwise;
};

template<>
struct closure<infill::geometry::polygon_inner<>>
{
    static const closure_selector value = open; // TODO: closed?, but when it is closed the order of the points changes
};

template<>
struct tag<infill::geometry::polygon_inner<>>
{
    using type = ring_tag;
};
